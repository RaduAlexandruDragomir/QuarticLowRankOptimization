function [Y infos] = gd_dist_completion(Is,Js,knownDists,Y,trueDists,params)
% [Y infos] = gd_dist_completion(Is,Js,knownDists,Y,trueDists,params)
%
...
% 
% Adapted by Radu-Alexandru Dragomir for the purpose of the paper
% Quartic First-Order methods for Low Rank optimization
%
% Modifications:
% added a trueDists parameter so as to monitor the root mean square error
% on the ground truth distances
...
Z = EIJ'*Y;infos.rmse = [RMSE(Y,trueDists)];
infos.rmse_time = [0.];
...
%         max_step = 2*stepSize * infos.sqnormgrad(end - 1) / gradientNorm;
    ...
    if verb,
        fprintf('[%0.4d] cost = %g, grad norm = %g, #linesearch = %g, step = %g\n',iter,newcost,gradientNorm, s, stepSize);
    end
    
    % Stopping criterion
    if newcost / mean_knownDists < tol || (cost - newcost)/cost < vtol,
        break;
    end
